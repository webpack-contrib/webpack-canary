  ...
  return new Promise((resolve, reject) => {
    ...
    childProcess.exec(commands.install, { cwd: ROOT_PATH }, (err, stdout, stderr) => {
      ...
      if (stderr) {
        ...
        const errors = reduce(outputted, function(validErrors, errLine) {
        ...
        if (errors.length > 0) {
          return reject(['Error output when installing', errors.join('\n')]);
        }
      }
      ...
      return childProcess.exec(commands.installDeps, { cwd: dependencySetup.installLocation }, (error) => {
        if (error) {
          return reject(['Error calling install command for dependency build', error]);
        }
