...
function getWebpackConfigPath(exampleDirectoryPath, groupedExamplePaths) {
  return flatten(groupedExamplePaths.map((examplePath) => {
 ...
 */
export function getDependencyExample(exampleDirectoryPath) {
  return (groupedExamplePaths) => {
    const webpackConfigFilePaths = getWebpackConfigPath(exampleDirectoryPath, groupedExamplePaths);
