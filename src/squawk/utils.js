...
export function createRunList(config) {
  const nestedRunList = map(
    config.versions,
    (dependencyVersions, webpackVersion) =>
      map(dependencyVersions, (dependencyVersion) => {
        return {
...
function updateResults(
  { webpack, dependency, examples, tests },
  results,
  update
) {
  ...
  each(results, (webpackResults, webpackVersion) => {
    ...
    if (every(webpackResults, (result) => result.success)) {
      loggerInstance.success(
        `No issues detected running ${keys(webpackResults).length} dependencies`
      );
    ...
    each(
      webpackResults,
      (
        { examples, tests, error: dependencyError, options = {} },
        dependencyVersion
      ) => {
        ...
        if (dependencyError) {
          ...
          table.push(
            [
              dependencyVersion,
              '-',
              `${failedMessage}${outputDependencyError}`,
            ],
            commandRow
          );
          return;
        ...
        each(examples, ({ name, error: exampleError }, index) => {
          ...
          const outputExampleError = exampleError
            ? convertErrorToString(exampleError)
            : '';
          const isFirst = index === 0;
          const nameColumn = isFirst
            ? [{ rowSpan: examples.length, content: dependencyVersion }]
            : [];
          table.push(
        ...
        each(tests, ({ error: testError }, index) => {
          ...
          const outputTestError = testError
            ? convertErrorToString(testError)
            : '';
          const isFirst = index === 0;
          const nameColumn = isFirst
            ? [{ rowSpan: tests.length, content: dependencyVersion }]
            : [];
          table.push(
            nameColumn.concat([
              `Test: ${options.test}`,
              `${testsStatus}${outputTestError}`,
            ])
          );
