...
async function runner() {
  ...
  const updateGauge = (webpack, value) =>
    !argv.verbose && gauge.show(`${webpack}`, value / runList.length);
  if (!argv.verbose) {
  ...
  try {
    for (const runItem of runList) {
      ...
      try {
        ...
        results = updateResultsForSuccess(
          { webpack, dependency, examples },
          results,
          canaryOptions
        );
      } catch (err) {
        ...
        results = updateResultsForFailure(
