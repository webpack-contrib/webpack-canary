...
describe('runDependencyWithWebpack', () => {
  ...
  describe('when readme file does not contain command', () => {
    ...
    it('executes default webpack compile', () => {
      ...
      expect(env.childProcessStub.spawn.firstCall.args[1]).to.deep.equal([
        '--config',
        './webpack.config.js',
      ]);
      expect(env.childProcessStub.spawn.firstCall.args[2]).to.deep.equal({
  ...
  describe('when executing', () => {
    ...
    describe('and error executing example', () => {
      ...
      it('calls the callback with error message', () =>
        env.runPromise.then(
          () => expect(true).to.equal(false),
          ({ err }) =>
            expect(err).to.deep.equal(['Failed to run example', 'Bad command'])
        ));
    });

    describe('and process is closed', () => {
      beforeEach(() => {
        env.processCloseHandler(1);
        env.clock.tick(110);
      });
        ...
        env.runPromise.then(
          ...
          ({ err }) =>
            expect(err).to.deep.equal([
              'Unable to detect successful compilation (1)',
            ])
        ));
    });
    ...
    describe('and example run causes an error', () => {
      ...
      it('calls the callback with error message and exits the example run', () => {
        ...
        return env.runPromise.then(
          ...
          ({ err }) =>
            expect(err).to.deep.equal([
              ...
              path.join(MODULES, 'webpack/bin/webpack.js'),
              ['--config', './webpack.config.js'],
              path.join(MODULES, 'raw-loader/examples'),
    ...
    describe('and webpack outputs a URL', () => {
      ...
      describe('which is internal', () => {
        ...
        it('does open the URL', () => {
          ...
          expect(env.childProcessStub.exec).to.have.been.calledWith(
            "phantomjs ./scripts/open-url.js 'http://localhost:8080'"
          );
        });
