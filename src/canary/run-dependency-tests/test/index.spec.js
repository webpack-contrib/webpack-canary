...
describe('runDependencyTests', () => {
  ...
  beforeEach(() => {
    ...
    env.runPromise = env.runDependencyTests(
      env.webpack,
      env.dependency,
      env.options
    );
  ...
  it('moves the dependency to testPath', async () => {
    ...
    expect(env.fsStub.renameSync).to.have.been.calledWith(
      env.dependency.installLocation,
      env.options.testPath
    );
  });
  ...
  it('clears the dependency webpack module', async () => {
    ...
    expect(env.rmfrMock.secondCall).to.have.been.calledWith(
      path.join(env.options.testPath, 'node_modules', 'webpack')
    );
  });
  ...
  it('symlinks webpack to dependency', async () => {
    ...
    expect(env.fsStub.symlinkSync).to.have.been.calledWith(
      env.webpack.installLocation,
      path.join(env.options.testPath, 'node_modules', 'webpack'),
      'junction'
    );
  });
  ...
  describe('running tests', () => {
    ...
    it('should fail if status code > 0', () => {
      ...
      return env.runPromise.then(
        ...
        (err) => expect(err.err).to.deep.equal(['Error calling command', error])
      );
    });

    it('should succeed if stderr, but code == 0', () => {
      env.childProcessStub.exec.firstCall.args[2](
        null,
        'Success',
