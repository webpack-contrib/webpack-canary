...
describe('Webpack Canary', () => {
  ...
  beforeEach(() => {
    ...
    env.runDependencyStub = () =>
      // eslint-disable-line arrow-body-style
      env.dependencyRunError ? Promise.reject(new Error()) : Promise.resolve();
  ...
  describe('when called', () => {
    describe('and webpack version is not valid', () => {
      ...
      describe('version is not for webpack', () => {
        beforeEach(() => {
          env.runnerPromise = env.webpackCanary(
            'underscore@1.2.3',
            null,
            env.options
          );
        ...
        it('reject with error', () =>
          env.runnerPromise.then(
            () => expect(true).to.equal(false),
            (error) =>
              expect(error.err).to.deep.equal([
                'Webpack version is not valid',
                'underscore@1.2.3',
              ])
          ));
  ...
  describe('Once examples have been retrieved', () => {
    ...
    describe('and examples are found', () => {
      describe('and not all examples compile', () => {
        ...
        it('runs each example with webpack', () =>
          env.runnerPromise.then(
            () => expect(true).to.equal(false),
