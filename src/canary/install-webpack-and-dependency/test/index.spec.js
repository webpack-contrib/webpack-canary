...
describe('installWebpackAndDependency', () => {
  ...
  beforeEach(() => {
    ...
    env.installPromise = env.installWebpackAndDependency(
      env.webpack,
      env.dependency,
      {},
      { timeout: 60000 }
  ...
  it('calls yarn add with correct values', () => {
    ...
    [
      ,
      ,
      env.webpackInstallCallback,
    ] = env.childProcessStub.exec.secondCall.args;
    expect(env.childProcessStub.exec).to.have.been.calledTwice;

    expect(env.childProcessStub.exec.secondCall).to.have.been.calledWith(
      'yarn add webpack@2.3.4 raw-loader@1.2.3',
      { cwd: ROOT_PATH }
    );
  });
  ...
  describe('when error installing', () => {
    ...
    it('calls the callback with error message', () =>
      env.installPromise.then(
        () => expect(true).to.equal(false),
        ({ err }) => expect(err).to.deep.equal(['Error calling command', error])
      ));
  });
    ...
    describe('which contain warnings', () => {
      beforeEach(async () => {
        env.webpackInstallCallback(
          null,
          installedOutput,
          'warning missing peer dependency'
        );
    ...
    describe('and examples install errors out', () => {
      ...
      it('calls the callback with error message', () =>
        env.installPromise.then(
          () => expect(true).to.equal(false),
          ({ err }) =>
            expect(err).to.deep.equal(['Error calling command', error])
        ));
    });
