 ...
 */
export default async function canaryRunner(
  webpackVersion,
  dependencyVersion,
  options
) {
  ...
  const dependencySetup = generateInstallObjectFor.dependency(
    dependencyVersion
  );
  if (isNull(dependencySetup)) {
    const error = new Error();
    error.err = [
      'Dependency details provided are not valid',
      dependencyVersion,
    ];
    throw error;
  }
  ...
  progressUpdate(
    `Installing ${chalk.bold(webpackSetup)} and ${chalk.bold(dependencySetup)}`,
    2,
    4
  );
  ...
  await installWebpackAndDependency(
    webpackSetup,
    dependencySetup,
  ...
  if (options.exampleDir) {
    progressUpdate(
      `Retrieving ${chalk.bold(dependencySetup.name)} examples`,
      3,
      4
    );
    ...
    dependencyExamples = await getDependencyExamples(
      webpackSetup,
      dependencySetup,
      options.exampleDir
    );
    ...
    logger.debug(
      `Finished get examples (${finishGetExamples - startGetExamples}ms)`
    );
    ...
    if (isEmpty(dependencyExamples)) {
      ...
      logger.debug('Unable to get any dependency examples');
      throw error;
    }

    progressUpdate(
      `Running ${chalk.bold(dependencySetup)} with ${chalk.bold(webpackSetup)}`,
      4,
      4 + dependencyExamples.length
    );
    ...
    for (const dependencyExample of dependencyExamples) {
      ...
      progressUpdate(
        `${dependencySetup.name} ${chalk.bold(dependencyExample.name)}`,
        4 + index,
        4 + dependencyExamples.length
      );
      ...
      try {
        ...
        logger.debug(
          `Finished run example (${finishRunExample - startRunExample}ms)`
        );
      } catch (err) {
  ...
  if (options.test) {
    try {
      logger.info(
        `Running ${chalk.bold(options.test)} tests for ${chalk.bold(
          dependencySetup
        )} with ${chalk.bold(webpackSetup)}...`
      );
      const testResults = await runDependencyTests(
        webpackSetup,
        dependencySetup,
        options
      );
